...
## 🧠 The Council of 6 + MoA

Our proprietary **Mixture-of-Agents (MoA) Consensus Engine** combines multiple AI models for robust trading decisions:
...
| 🤖 **Gemini 2.5 Flash** | Context & Memory | Google |
| 🔬 **Mistral 7B** | Technical Analysis | OpenRouter |
| � **Titan HNN** | Local Fallback (93.8% accuracy) | **Self-Hosted** |

### Groq Llama Fleet (4 Specialized Agents)

We leverage **Groq's ultra-fast inference** with 4 Llama 3.1 8B instances, each with a specialized role:

| Agent | Role | Temperature |
|-------|------|-------------|
| ⚡ **Quant Agent** | Technical pattern recognition | 0.1 |
| 📊 **Risk Agent** | Risk assessment & position sizing | 0.1 |
| 🎯 **Groq Agent** | Momentum & trend analysis | 0.1 |
| � **Meta Agent** | Aggregates all signals (MoA Orchestrator) | 0.2 |

### MoA Architecture

```
┌─────────────────────────────────────────────────────────────────────┐
│                    MARKET DATA: RSI=35, F&G=28, OFI=+15%            │
└───────────────────────────────────┬─────────────────────────────────┘
                                    ▼
    ┌─────────────────────────────────────────────────────────────────┐
    ...
    ├──────────┬──────────┬──────────┬──────────┬──────────┬──────────┤
    │  🔮      │  🤖      │  ⚡      │  📊      │  🎯      │  🔬      │
    │ DeepSeek │ Gemini   │ Quant    │ Risk     │ Groq     │ Mistral  │
    │  BUY     │  HOLD    │  BUY     │  BUY     │  BUY     │  HOLD    │
    └──────────┴──────────┴──────────┴──────────┴──────────┴──────────┘
                                    ▼
    ┌─────────────────────────────────────────────────────────────────┐
    │                 LAYER 2: META AGGREGATOR (MoA)                  │
    │              🧠 Llama 3.1 8B Meta Agent (Groq)                  │
    │         Weighs all responses + historical performance           │
    └───────────────────────────────┬─────────────────────────────────┘
                                    ▼
                         🎯 CONSENSUS: BUY (72%)
                                    ▼
    ┌─────────────────────────────────────────────────────────────────┐
    │              LAYER 3: SAFETY GUARDIAN (LOCAL)                   │
    │  🦁 Titan HNN validates signal | Math Guardian checks thresholds│
    └─────────────────────────────────────────────────────────────────┘
```

> **Fallback Chain:** If Groq is rate-limited → Gemini decides → If Gemini fails → Titan HNN (100% uptime)
